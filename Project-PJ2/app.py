...
# Создание вкладок
# -----------------------------------------------------
tab_show_data, tab_charts, tab_predict, tab_comfort_index, tab_default_app = st.tabs(["**Просмотр таблицы**","**Работа с графиками**","**Прогноз**","**Индекс комфорта**","**default_app**"])
...
with tab_predict:
    #Заголовок
    st.subheader("Прогнозирование на основе скользящего среднего", text_alignment="center", divider=True)
    ...
    if not selectbox_predict_data or not selectbox_predict_city:
        ...
        # Фильтрация по дате. 
        # Так как в новой версии в данных у нас теперь уже содержится прогноз на 7 дней,
        # получаемый в fetch_and_store.py через API, для SMA прогноза нужно убрать API прогнозные данные.
        # Для этого оставляем только данные с датой ДО сегодняшнего дня.       

        today = date.today()
        city_df = city_df.filter(pl.col('date') < today)
        ...
        result_df = pd.DataFrame({
            ...
            'Тип': ['Реальные данные' if date <= last_real_date else 'Прогноз SMA' for date in forecast_series.index]
        })
        ...
        fig = px.line(
            ...
            title=f"Реальные данные и прогноз SMA: {selectbox_predict_data} в {selectbox_predict_city}",
        ...
        #--------------------------------------------------
        # График: Прогноз по данным из API
        #--------------------------------------------------
        
        # Извлекаем ВСЕ данные по городу из filtered_df
        full_city_df = filtered_df.filter(pl.col("city") == selectbox_predict_city).to_pandas()
        full_city_df["date"] = pd.to_datetime(full_city_df["date"])
        full_city_df = full_city_df.sort_values("date").reset_index(drop=True)

        # Определяем последнюю реальную дату
        last_real_date = today - pd.Timedelta(days=1)
        full_city_df["Тип"] = full_city_df["date"].apply(
            lambda d: "Реальные данные" if d.date() < today else "Прогноз от API"
        )

        # Формируем DataFrame для графика
        plot_df = full_city_df[["date", target_col, "Тип"]].copy()
        plot_df.rename(columns={"date": "Дата", target_col: "Значение"}, inplace=True)
        ...
        fig_api = px.line(
            ...
            color_discrete_map={
                "Реальные данные": "blue",
                "Прогноз от API": "orange"
            },
            line_dash="Тип",
            line_dash_map={
                "Реальные данные": "solid",
                "Прогноз от API": "solid"  
            },
            markers=False,
            title=f"Реальные данные и прогноз от API: {selectbox_predict_data} в {selectbox_predict_city}",
            labels={"Значение": f"{selectbox_predict_data} ({unit})", "Дата": "Дата"}
        )
    ...
    # На вкладке исходное приложение из учебных материалов. Копируем сюда как есть, просто, что бы было.
    with tab_default_app:
        ...
        # Выбор города
        ###sorted(filtered_df['city'].unique().to_list()),
        cities = sorted(filtered_df["city"].unique().to_list())
        selected_city = st.selectbox("Выберите город", cities)

        ###city_df = filtered_df.filter(pl.col('city') == selectbox_predict_city) 
        city_data = filtered_df.filter(pl.col("city") == selected_city)
        ...
        historical_data = city_data.filter(pl.col("date") < today)
        forecast_data = city_data.filter(pl.col("date") >= today)
    ...
    # Вкладка "Индекс комфорта" код ниже собран из учебных материалов
    with tab_comfort_index:

        # Выбор города
        selectbox_comfort_index_selected_city = st.selectbox(
            label="Выберите город",
            index=0,
            options=sorted(filtered_df["city"].unique().to_list()),
            placeholder="Выберите данные для прогноза ...",
            key="selectbox_comfort_index_selected_city"
        )

        selectbox_comfort_index_city_data = filtered_df.filter(pl.col("city") == selectbox_comfort_index_selected_city)

        # График температуры (остаётся как есть)
        fig_temp = px.line(
            selectbox_comfort_index_city_data.to_pandas(),
            x="date",
            y="avg_temp",
            title=f"Средняя температура в {selectbox_comfort_index_selected_city}",
            labels={"avg_temp": "Температура (°C)", "date": "Дата"}
        )
        ...
        fig_comfort = px.line(
            ...
            x="date",
            y="comfort_index",
            title=f"Индекс комфорта в {selectbox_comfort_index_selected_city} (чем выше — тем комфортнее)",
        ...
        col3.metric("Средний комфорт", f"{avg_comfort:.1f}")

        # Таблица последних данных (автоматически покажет новую колонку)
        st.subheader("Последние данные")
        st.dataframe(city_data.tail(10).to_pandas())