...
#Задание 21.10.1 (External resource)
print("\nЗадание 21.10.1 (External resource) -->\n")

#task — словарь вида {'name': str, 'done': bool}
...
class TaskList:
    ...
    def __is_task_in_list(self, task):
        for task_dict in self.__task_list:            
            if task in task_dict.values():
                return True            
        return False
    ...
    def remove_task(self, task_name):
        if  self.__is_task_in_list(task_name):            
            for i,d in enumerate(self.__task_list):                
                if d['name'] == task_name:
